...
async function loadCards() {
  ...
  // === ANTI-BUG: Валидация данных ===
  let rawCards;
  if (typeof validateDataFile === 'function') {
    rawCards = validateDataFile(payload);
  } else {
    // Fallback если validation.js не подключён
    rawCards = Array.isArray(payload) ? payload : (Array.isArray(payload.cards) ? payload.cards : []);
  }
